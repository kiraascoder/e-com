...
@section('content')
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($users as $user)
                        ...
                        <tr class="hover:bg-gray-50" data-role="{{ $role }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $roleColors[$role] ?? 'bg-gray-100 text-gray-800' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-3">
                                    ...
                                    <button type="button" class="btnEdit text-indigo-600 hover:text-indigo-900"
                                        title="Edit" aria-label="Edit"
                                        data-action="{{ route('admin.users.update', $user->id) }}"
                                        data-nama="{{ $user->name }}" data-email="{{ $user->email }}"
                                        data-role="{{ $user->role }}" data-bidang-id="{{ $user->bidang_id }}">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M11 4h2a2 2 0 012 2v2m-2-4L4 13v7h7l9-9m-5-5l5 5" />
                                        </svg>
                                    </button>

                                    <button type="button" class="btnHapus text-red-600 hover:text-red-900" title="Hapus"
                                        aria-label="Hapus" data-id="{{ $user->id }}" data-nama="{{ $user->name }}">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div id="modalTambahUser" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div
                class="modal-content bg-white rounded-xl shadow-2xl max-w-md w-full transform scale-95 opacity-0 transition-all duration-300">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">Tambah User Baru</h3>
                        <button type="button" class="btnCloseModal text-gray-400 hover:text-gray-600"
                            data-target="modalTambahUser" aria-label="Tutup">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <select name="role" id="roleSelect"
                                    ...
                                    <option value="ketua_bidang" {{ old('role') === 'ketua_bidang' ? 'selected' : '' }}>
                                        Ketua Bidang</option>
                                    <option value="pegawai" {{ old('role') === 'pegawai' ? 'selected' : '' }}>Pegawai
                                    </option>
                                    <option value="kepala_dinas" {{ old('role') === 'kepala_dinas' ? 'selected' : '' }}>
                                        Kepala Dinas</option>
                                </select>
                                @error('role')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button type="button" class="btnCloseModal text-gray-400 hover:text-gray-600"
                            data-target="modalEditUser" aria-label="Tutup">
                            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
                            </svg>
    ...
    <div id="modalKonfirmasiHapus" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div
                class="modal-content bg-white rounded-xl shadow-2xl max-w-sm w-full transform scale-95 opacity-0 transition-all duration-300">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-600 mb-4">
                            Apakah Anda yakin ingin menghapus user <strong id="namaUserHapus"></strong>? Tindakan ini tidak
                            dapat dibatalkan.
                        </p>
                    </div>
                    <form id="formHapusUser" method="POST" action="#">
                        ...
                        <div class="flex justify-end gap-3">
                            <button type="button"
                                class="btnCloseModal px-4 py-2 text-gray-700 border border-gray-300 rounded-md hover:bg-gray-50 transition"
                                data-target="modalKonfirmasiHapus">Batal</button>
                            <button type="submit"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (roleCreate) {
                ...
                roleCreate.addEventListener('change', function() {
                    toggleBidang(this, 'formBidangCreate');
                });
            }
            ...
            if (roleEdit) {
                roleEdit.addEventListener('change', function() {
                    toggleBidang(this, 'formBidangEdit');
                });
            }
            ...
            @if ($errors->any() && old('_token'))
                @if (old('form_type') === 'user_create')
                    ...
                    setTimeout(() => {
                        if (roleCreate) toggleBidang(roleCreate, 'formBidangCreate');
                    }, 0);
                @elseif (old('form_type') === 'user_edit')
                    showModal('modalEditUser');
                    setTimeout(() => {
                        if (roleEdit) toggleBidang(roleEdit, 'formBidangEdit');
                    }, 0);
                @endif
            @endif
