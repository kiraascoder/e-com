    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            @include('partials.metric-card', [
                ...
                'value' => $metrics['laporan_bidang'] ?? '67',
            ...
            @include('partials.metric-card', [
                ...
                'value' => 'Rp ' . ($metrics['budget_used'] ?? '12.8M'),
                'subtitle' => 'Anggaran bidang',
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                @include('partials.quick-action', [
                    ...
                    'description' => 'Tugaskan laporan ke tim',
                    'bgColor' => 'bg-green-100',
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <div class="space-y-4">
                    @foreach ($team_status ?? [] as $team)
                        <div class="bg-white rounded-lg shadow p-6">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-medium text-gray-900">{{ $team['name'] ?? 'Tim Infrastruktur' }}
                                </h3>
                                <span
                                    class="px-2 py-1 text-xs font-medium rounded-full {{ $team['status'] === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <span class="font-medium">{{ $team['members'] ?? '5' }} orang</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Tugas Aktif:</span>
                                    <span class="font-medium">{{ $team['active_tasks'] ?? '3' }} tugas</span>
                                </div>
                            </div>
                            <div class="mt-4">
                                <div class="flex justify-between items-center mb-1">
                                    <span class="text-xs text-gray-600">Progress Bulan Ini</span>
                                    <span class="text-xs text-gray-600">{{ $team['progress'] ?? '85' }}%</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-600 h-2 rounded-full"
                                        style="width: {{ $team['progress'] ?? '85' }}%"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
